...
| Wrist Commands | Hold X on foot. After the commands appear, center the right stick, point toward an available command, and confirm with A. Release X to close. On horseback use LT+X and keep LT held because X alone is gallop. Native D-Horse call/Stay back and distraction knock were exercised in SIM with earlier bindings. Contextual interrogation and other buddies still need target interactions. |
| Equipment and NVG | Left trigger unfolds the wrist display without selecting anything; a fresh right-stick flick chooses a category. NVG is the upward item slot; selecting None turns it off. NVG is separate from binoculars. |
...
| Binoculars/scopes | Hold right B for about 0.3 seconds to equip the stereo binoculars in the right hand; a fresh B tap stows them. Releasing B or grip keeps them equipped. Left-stick click switches 2×/4×; right-stick click dives; right trigger marks and A clears. Native analysis/intel and per-weapon scopes remain unfinished. |
