...
Physical headset acceptance has failed: the newer build produced an incompatible landscape between eyes, and the attempted older rollback produced garbled limbs, poor sharpness and flat or inconsistent UI depth. Neither build is an accepted recovery. The latest SIM candidate removes the sampled sky rectangle by cropping a centered render back to each eye's requested optics, and puts equipment cards above the left wrist. [Current evidence and limitations](docs/SKY_CROP_REVIEW.md) include a short recording; physical stereo acceptance remains unresolved. This is not a polished release.
