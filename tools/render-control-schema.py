...
import html,importlib.util,json,pathlib,textwrap
art_spec=importlib.util.spec_from_file_location('field_art',pathlib.Path(__file__).with_name('field-art.py'))
...
art.header(parts)
rect(40,325,12,84,'#af2624')
text(76,368,schema['title'],44,900);text(77,404,schema['edition'],20)
y=449
...
svg=''.join(art.begin(1440,y+25,'MGS5VR complete controller modes — '+schema['edition']))+''.join(parts)+'</g></svg>'
